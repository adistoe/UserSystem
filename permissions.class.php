...
class Permissions
{
        ...
        if ($this->db->query('
            ...
            CREATE TABLE ' . $this->dbTables['group_permissions'] . '(
                ...
                CONSTRAINT GP_FK_PID FOREIGN KEY (PID)
                    REFERENCES ' . $this->dbTables['permissions'] . ' (PID)
            );
    ...
    public function getGroupPermissions($gid)
    {
        ...
        $stmt = $this->db->prepare('
            SELECT
                ...
                p.name,
                description
            FROM ' . $this->dbTables['groups'] . ' AS g
                JOIN ' . $this->dbTables['group_permissions'] . ' AS gp
                    ON g.GID = gp.GID
                JOIN ' . $this->dbTables['permissions'] . ' AS p
                    ON gp.PID = p.PID
            WHERE
                g.GID = :GID
        ');
    ...
    public function getPermission($pid)
    {
        ...
        $stmt = $this->db->prepare('
            SELECT
                *
            FROM ' . $this->dbTables['permissions'] . '
            WHERE
                PID = :PID
        ');
    ...
    public function getUserGroups(
        ...
        $stmt = $this->db->prepare("
            SELECT
                g.GID,
                name
            FROM " . $this->dbTables['user_groups'] . " AS ug
                JOIN " . $this->dbTables['groups'] . " AS g
                    ON ug.GID = g.GID
            WHERE
                ug.UID = :UID
            ORDER BY
    ...
    public function groupCreate($name)
    {
        if (strlen($name) > 0) {
            ...
            $stmt = $this->db->prepare('
                INSERT INTO ' . $this->dbTables['groups'] . '(
                    name
                ) VALUES(
                    :name
                )
    ...
    public function groupDelete($gid)
    {
        $stmt = $this->db->prepare('
            DELETE
                ...
                gp
            FROM ' . $this->dbTables['groups'] . ' AS g
                LEFT JOIN ' . $this->dbTables['user_groups'] . ' AS ug
                    ON g.GID = ug.GID
                LEFT JOIN ' . $this->dbTables['group_permissions'] . ' AS gp
                    ON g.GID = gp.GID
            WHERE
                g.GID = :GID
        ');
    ...
    public function groupEdit($gid, $name)
    {
        ...
        $stmt = $this->db->prepare('
            UPDATE ' . $this->dbTables['groups'] . ' SET
                name = :name
            WHERE
                GID = :GID
        ');
    ...
    public function groupPermissionGrant($gid, $pid)
    {
        $stmt = $this->db->prepare('
            SELECT
                PID
            FROM ' . $this->dbTables['group_permissions'] . '
            WHERE
                GID = :GID AND
                PID = :PID
            ');
        ...
        $stmt = $this->db->prepare('
            INSERT INTO ' . $this->dbTables['group_permissions'] . '(
                GID,
                PID
            ) VALUES(
                :GID,
    ...
    {
        $stmt = $this->db->prepare('
            DELETE FROM ' . $this->dbTables['group_permissions'] . '
            WHERE
                GID = :GID AND
                PID = :PID
            ');
    ...
    public function permissionCreate($name, $description)
    {
        if (strlen($name) > 0 && strlen($description) > 0) {
            ...
            $stmt = $this->db->prepare('
                INSERT INTO ' . $this->dbTables['permissions'] . '(
                    name,
                    description
                ) VALUES(
                    :name,
    ...
    public function permissionDelete($pid)
    {
        $stmt = $this->db->prepare('
            DELETE
                p,
                gp
            FROM ' . $this->dbTables['permissions'] . ' AS p
                LEFT JOIN ' . $this->dbTables['group_permissions'] . ' AS gp
                    ON p.PID = gp.PID
            WHERE
                p.PID = :PID
    ...
    public function permissionEdit($pid, $name, $description)
    {
        ...
        $stmt = $this->db->prepare('
            UPDATE ' . $this->dbTables['permissions'] . ' SET
                name = :name,
    ...
    public function userGroupAdd($uid, $gid)
    {
        ...
        if ($row_user = $stmt_user->fetchObject() &&
            $row_group = $stmt_group->fetchObject()
        ) {
            $stmt = $this->db->prepare('
                INSERT INTO ' . $this->dbTables['user_groups'] . '(
                    UID,
                    GID
                ) VALUES(
                    :UID,
    ...
    {
        $stmt = $this->db->prepare('
            DELETE FROM ' . $this->dbTables['user_groups'] . '
            WHERE
                UID = :UID AND
                GID = :GID
        ');
